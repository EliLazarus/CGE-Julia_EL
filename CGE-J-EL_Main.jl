...
HHSavi = YiIn - ConsBudgi #Savings set by the difference - in the data
TotSavi = HHSavi #Savings starts
mps = HHSavi/YiIn #marginal propensity to save (savings as a proportion of income)
...
@variable(CGE_EL, Le, start = Lei, lower_bound = 0, lower_bound = 0.001 * Lei)
...
@NLconstraint(CGE_EL, EPhil, ((w/PrInd)/(wLi/PrIndi)-1) == Phili * ((Unempl/Le)/(Unempli/Lei)-1)) # Wage Curve
...
@NLconstraint(CGE_EL, EHHSav, HHSav == mps * HHI) # Household Savings
...
@NLconstraint(CGE_EL, ENoProf[i = sectors], Pr_Commods[i] * YOut[i] == r * Kd_f[i] + w * Ld_f[i] +
 sum(TechCf[i,j] * YOut[i] * Pr_Commods[j] for j in sectors )) #Competitive Equlibrium, no profit (value of output=production)
...
HHU = prod((JuMP.value(Commodsd_HH[i])-HHCsubsist[i])^HHUlesexp[i] for i in sectors)
